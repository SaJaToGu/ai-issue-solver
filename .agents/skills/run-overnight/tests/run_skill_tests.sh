#!/usr/bin/env bash
# Wrapper, der die unittest-Suite des run-overnight-Skills ausführt.
#
# Verwendung:
#   bash .agents/skills/run-overnight/tests/run_skill_tests.sh
#   bash .agents/skills/run-overnight/tests/run_skill_tests.sh --verbose

set -euo pipefail

# tests/ → run-overnight/ → skills/ → .agents/ → REPO_ROOT
ROOT_DIR="$(cd "$(dirname "$0")/../../../.." && pwd)"
TESTS_DIR="$ROOT_DIR/.agents/skills/run-overnight/tests"

PYTHON="${PYTHON:-python}"

echo "=== run-overnight Skill-Tests ==="
