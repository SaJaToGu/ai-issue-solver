#!/usr/bin/env bash
# Wrapper, der die unittest-Suite des solve-issues-Skills ausführt.
#
# Verwendung:
#   bash .agents/skills/solve-issues/tests/run_skill_tests.sh
#   bash .agents/skills/solve-issues/tests/run_skill_tests.sh --verbose

set -euo pipefail

# tests/ → solve-issues/ → skills/ → .agents/ → REPO_ROOT
ROOT_DIR="$(cd "$(dirname "$0")/../../../.." && pwd)"
TESTS_DIR="$ROOT_DIR/.agents/skills/solve-issues/tests"

PYTHON="${PYTHON:-python}"

echo "=== solve-issues Skill-Tests ==="
