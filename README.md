...
es, was manuell geprüft werden sollte. Ohne `--apply` läuft es als Dry-Run.
...
3. Konfiguration kopieren und anpassen
   ```bash
   cp config/config.example.env config/.env
   # .env mit deinen Werten befüllen (NIEMALS committen!)
   ```

Für detaillierte Setup-Anleitungen siehe [Provider-Einrichtung](docs/SETUP_AIDER.md).

---

## Schnellstart

1. Erste Analyse starten:
   ```bash
   python scripts/analyze_repos.py --user DeinGitHubUsername --output reports/analysis.json
   ```

2. Issues erstellen (zuerst im Dry-Run-Modus):
   ```bash
   python scripts/create_issues.py --report reports/analysis.json --dry-run
   ```

3. Issues mit KI lösen (Dry-Run):
   ```bash
...
| Befehl | Beschreibung |
|--------|-------------|
| `python scripts/analyze_repos.py --user DeinGitHubUsername` | Analysiert alle Repos eines GitHub-Nutzers
| `python scripts/create_issues.py --report reports/analysis.json --dry-run` | Zeigt Issues an ohne sie zu erstellen
| `python scripts/solve_issues.py --model codex --dry-run` | Löst Issues mit KI (Simulation)
| `python scripts/github_summary.py` | Zeigt Statusübersicht
| `python scripts/status_dashboard.py` | Erstellt HTML-Dashboard mit Run-Reports

Für detaillierte Anleitungen siehe:
- [Workflow-Dokumentation](docs/WORKFLOW.md)
- [Provider-Einrichtung](docs/SETUP_AIDER.md)
...
der Bezug zum Issue sichtbar bleibt, zum Beispiel `ai/fix-issue-{nummer}`.

Für detaillierte Workflow-Anleitungen siehe [docs/WORKFLOW.md](docs/WORKFLOW.md).
...
Um die Lesbarkeit der README zu erhalten, bitte detaillierte Anleitungen, Konfigurationen und Workflow-Beschreibungen in der [Dokumentation](docs/) ablegen. Die README sollte nur als Quickstart und Übersicht dienen.
